(function () {
    ...
    function initMap(app) {
        ...
        var menuToggle = app.querySelector('[data-arpm-menu-toggle]');
        var menuClose = app.querySelector('[data-arpm-menu-close]');
        var layerMenu = app.querySelector('[data-arpm-layer-menu]');
        var layerGrid = app.querySelector('[data-arpm-layer-grid]');
        var currentLayerText = app.querySelector('[data-arpm-current-layer]');
        var previousButton = app.querySelector('[data-arpm-previous]');
        var playButton = app.querySelector('[data-arpm-play]');
        var nextButton = app.querySelector('[data-arpm-next]');
        var validity = app.querySelector('[data-arpm-validity]');
        var lead = app.querySelector('[data-arpm-lead]');
        var run = app.querySelector('[data-arpm-run]');
        var generated = app.querySelector('[data-arpm-generated]');
        var stale = app.querySelector('[data-arpm-stale]');
        var viewport = app.querySelector('[data-arpm-viewport]');
        var weatherCanvas = app.querySelector('[data-arpm-weather]');
        var vectorCanvas = app.querySelector('[data-arpm-vectors]');
        var labelsCanvas = app.querySelector('[data-arpm-labels]');
        ...
        var mapTitle = app.querySelector('[data-arpm-map-title]');
        var mapRun = app.querySelector('[data-arpm-map-run]');
        var mapDate = app.querySelector('[data-arpm-map-date]');
        var loading = app.querySelector('[data-arpm-loading]');
        var errorBox = app.querySelector('[data-arpm-error]');
        var slider = app.querySelector('[data-arpm-slider]');
        var timeline = app.querySelector('[data-arpm-timeline], .arpm-timeline');
        var periodSelector = app.querySelector('[data-arpm-period]');
        var dualRange = app.querySelector('[data-arpm-dual-range]');
        var periodStartSlider = app.querySelector('[data-arpm-period-start]');
        var periodEndSlider = app.querySelector('[data-arpm-period-end]');
        var periodTitle = app.querySelector('[data-arpm-period-title]');
        var periodSummary = app.querySelector('[data-arpm-period-summary]');
        var periodStartLabel = app.querySelector('[data-arpm-period-start-label]');
        var periodEndLabel = app.querySelector('[data-arpm-period-end-label]');
        var legend = app.querySelector('[data-arpm-legend]');
        var zoomIn = app.querySelector('[data-arpm-zoom-in]');
        var zoomOut = app.querySelector('[data-arpm-zoom-out]');
        var reset = app.querySelector('[data-arpm-reset]');
        var fullscreen = app.querySelector('[data-arpm-fullscreen]');
        var zoomLevel = app.querySelector('[data-arpm-zoom-level]');
        var probe = app.querySelector('[data-arpm-probe]');
        var probeValue = app.querySelector('[data-arpm-probe-value]');
        var probeLabel = app.querySelector('[data-arpm-probe-label]');
        var toolButtons = app.querySelectorAll('[data-arpm-tool]');
        var toolHint = app.querySelector('[data-arpm-tool-hint]');
        var advancedTools = app.querySelector('[data-arpm-advanced-tools]');
        var captureButton = app.querySelector('[data-arpm-capture]');
        var pinButton = app.querySelector('[data-arpm-pin]');
        var diagramPopup = app.querySelector('[data-arpm-diagram-popup]');
        var diagramTitle = app.querySelector('[data-arpm-diagram-title]');
        var diagramBody = app.querySelector('[data-arpm-diagram-body]');
        var diagramStatus = app.querySelector('[data-arpm-diagram-status]');
        var diagramClose = app.querySelector('[data-arpm-diagram-close]');
        ...
        function pinProbeAt(clientX, clientY) {
            ...
            pinnedElement.className = 'arpm-probe arpm-probe-pinned';
            ...
            close.type = 'button';
            close.className = 'arpm-probe-pin-close';
        ...
        function setToolMode(mode) {
            ...
            toolButtons.forEach(function (button) {
                var active = button.dataset.arpmTool === toolMode;
            ...
            if (toolMode === 'diagram') {
                setToolHint('Cliquez sur la carte pour afficher le diagramme ARPEGE du point choisi.');
            } else {
                setToolHint('');
                closeDiagram();
            }
        ...
        function captureImage() {
            ...
            canvas.toBlob(function (blob) {
                ...
                var layerLabel = manifest && manifest.layers && manifest.layers[currentLayer]
                    ...
                    : currentLayer;
                var slug = String(layerLabel || 'arpege').toLowerCase()
                    .normalize('NFD').replace(/[̀-ͯ]/g, '')
                    .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
                link.href = url;
                link.download = 'arpege-' + (slug || 'carte') + '-' + Date.now() + '.png';
        ...
        function renderDiagramChart(name, forecastRows, columnIndex, pointIndex) {
            ...
            svg.setAttribute('class', 'arpm-diagram-svg');
            svg.setAttribute('role', 'img');
            svg.setAttribute('aria-label', 'Diagramme ARPEGE pour ' + name);
            ...
            rains.forEach(function (value, index) {
                ...
                rect.setAttribute('class', 'arpm-diagram-rain');
            ...
            if (points.length > 1) {
                ...
                polyline.setAttribute('class', 'arpm-diagram-temp');
            ...
            [0, count - 1].forEach(function (index) {
                ...
                text.setAttribute('class', 'arpm-diagram-axis');
            ...
            caption.className = 'arpm-diagram-caption';
            caption.textContent = 'Température (ligne) et précipitations horaires (barres) — prochaines échéances ARPEGE.';
        ...
        function applyMapAspectRatio(bounds) {
            ...
            viewport.style.setProperty('--arpm-map-ratio', String(ratio));
        }
        ...
        function refreshLayerMenu() {
            ...
            layerGrid.querySelectorAll('[data-arpm-layer-key]').forEach(function (button) {
                var active = button.dataset.arpmLayerKey === currentLayer;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-pressed', active ? 'true' : 'false');
            });
        }
        ...
        function buildLayerMenu() {
            ...
            var columns = [0, 1, 2].map(function () {
                ...
                column.className = 'arpm-layer-column';
            ...
            groupOrder.forEach(function (group) {
                ...
                section.className = 'arpm-layer-group';
                ...
                grouped[group].forEach(function (entry) {
                    ...
                    button.type = 'button';
                    button.className = 'arpm-layer-option' +
                        (essential ? '' : ' arpm-layer-option-secondary');
                    if (!essential) {
                        button.hidden = !showSecondaryLayers;
                    }
                    button.dataset.arpmLayerKey = entry.key;
            ...
            if (hasSecondary) {
                ...
                moreButton.className = 'arpm-layer-more';
                moreButton.dataset.arpmLayerMore = '1';
        ...
        function buildLegend() {
            ...
            strip.className = 'arpm-legend-strip';
            layer.stops.forEach(function (stop) {
                var item = document.createElement('div');
                item.className = 'arpm-legend-stop';
        ...
        function updatePeriodControls() {
            ...
            if (dualRange && dualRange.style && dualRange.style.setProperty) {
                ...
                dualRange.style.setProperty(
                    '--arpm-period-start', (periodStart / maximum * 100) + '%'
                );
                dualRange.style.setProperty(
                    '--arpm-period-end', (periodEnd / maximum * 100) + '%'
                );
            }
        }
        ...
        function renderStep(index) {
            ...
            mapTitle.textContent = (layer ? layer.label : 'Carte ARPEGE') +
        ...
        app.addEventListener('arpm:focus-location', function (event) {
        ...
        toolButtons.forEach(function (button) {
            button.addEventListener('click', function () {
                setToolMode(button.dataset.arpmTool);
            });
        });
        if (captureButton) {
        ...
        viewport.addEventListener('pointerdown', function (event) {
            if (event.target.closest('button, .arpm-diagram-popup, .arpm-probe-pinned')) {
                return;
            }
            hideProbe();
            tapStart = {
        ...
        if (!baseUrl) {
            showError('Adresse des données ARPEGE non configurée.');
            return;
        }
        ...
        fetchJson(baseUrl + '/maps/index.json')
            ...
            .catch(function (error) {
                showError('Les cartes ARPEGE ne sont pas encore publiées : ' + error.message);
            });
    }

    whenReady(function () {
        document.querySelectorAll('[data-arpm-app]').forEach(initMap);
    });
}());
