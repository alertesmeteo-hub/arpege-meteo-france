(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'arp-temp-extreme'; }
        if (value >= 30) { return 'arp-temp-hot'; }
        if (value >= 22) { return 'arp-temp-warm'; }
        if (value >= 12) { return 'arp-temp-mild'; }
        if (value >= 4) { return 'arp-temp-cool'; }
        return 'arp-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'arp-gust-danger'; }
        if (value >= 60) { return 'arp-gust-strong'; }
        if (value >= 40) { return 'arp-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'arp-wind-calm'; }
        if (value >= 80) { return 'arp-wind-violent'; }
        if (value >= 60) { return 'arp-wind-strong'; }
        if (value >= 40) { return 'arp-wind-fresh'; }
        if (value >= 20) { return 'arp-wind-moderate'; }
        return 'arp-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'arp-rain arp-rain-intense'; }
        if (value >= 5) { return 'arp-rain arp-rain-strong'; }
        if (value >= 2) { return 'arp-rain arp-rain-moderate'; }
        if (value >= 0.5) { return 'arp-rain arp-rain-light'; }
        return 'arp-rain arp-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'arp-chart-day-band arp-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'arp-chart-day-strip arp-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'arp-chart-day-separator arp-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'arp-chart-day-label arp-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'arp-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'arp-chart-metric';
            item.appendChild(textSpan(metric.label, 'arp-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'arp-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'arp-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'arp-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-arp-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-arp-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'arp-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'arp-chart-empty'));
            return;
        }
        ...
        svg.classList.add('arp-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'arp-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'arp-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'arp-chart-grid arp-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'arp-chart-axis arp-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'arp-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'arp-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'arp-chart-hitpoint ' : 'arp-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-arp-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'arp-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.arp-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'arp-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'arp-chart-axis arp-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'arp-chart-rain-zero'; }
        if (value >= 15) { return 'arp-chart-rain-intense'; }
        if (value >= 5) { return 'arp-chart-rain-strong'; }
        if (value >= 2) { return 'arp-chart-rain-moderate'; }
        if (value >= 0.5) { return 'arp-chart-rain-light'; }
        return 'arp-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'arp-chart-axis arp-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'arp-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-arp-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'arp-chart-line arp-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'arp-chart-hitpoint arp-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-arp-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'arp-chart-rain-bar arp-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions ARPEGE';

        var input = app.querySelector('.arp-city-input');
        var locateButton = app.querySelector('[data-arp-locate]');
        var results = app.querySelector('.arp-search-results');
        var status = app.querySelector('.arp-search-status');
        var generalBody = app.querySelector('[data-arp-body-general]');
        var stormBody = app.querySelector('[data-arp-body-storms]');
        var stormSummary = app.querySelector('[data-arp-storm-summary]');
        var snowBody = app.querySelector('[data-arp-body-snow]');
        var snowSummary = app.querySelector('[data-arp-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-arp-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-arp-panel]'));
        var title = app.querySelector('[data-arp-title]');
        var altitudeLine = app.querySelector('[data-arp-altitude]');
        var meta = app.querySelector('[data-arp-meta]');
        var generated = app.querySelector('[data-arp-generated]');
        var stale = app.querySelector('[data-arp-stale]');
        var stormTopScroll = app.querySelector('[data-arp-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-arp-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.arp-storm-table');
        var snowTopScroll = app.querySelector('[data-arp-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-arp-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.arp-snow-table');
        var chartTemperature = app.querySelector('[data-arp-chart-temperature]');
        var chartPressure = app.querySelector('[data-arp-chart-pressure]');
        var chartRain = app.querySelector('[data-arp-chart-rain]');
        var chartWind = app.querySelector('[data-arp-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-arp-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-arp-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-arp-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-arp-chart-title-wind]');
        var rainTotal = app.querySelector('[data-arp-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'arp-day-cell arp-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'arp-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'arp-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('arp-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.arpTab === view;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            panels.forEach(function (panel) {
                panel.hidden = panel.dataset.arpPanel !== view;
            });
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-arpm-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('arpm:focus-location', {
        ...
        tabs.forEach(function (button) {
            button.addEventListener('click', function () {
                setActiveView(button.dataset.arpTab || 'general');
            });
        });
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'arp-loading arp-load-error' : 'arp-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('arp-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('arp-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national ARPEGE invalide');
                    }
                    indexData = payload;
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • diagnostics orages + neige ARPEGE' : '');
        ...
        function loadDepartment(code) {
            ...
            if (!indexData || !indexData.departments[normalized]) {
                return Promise.reject(new Error('Département hors couverture ARPEGE'));
            }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'arp-hazard arp-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance ARPEGE disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('arp-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'arp-hour arp-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'arp-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'arp-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'arp-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'arp-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('arp-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'arp-hour arp-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'arp-thunder-risk arp-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'arp-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'arp-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'arp-cape-high' : (finite(cape) && cape >= 500 ? 'arp-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'MUCAPE instantanée directement produite par ARPEGE ; les valeurs quasi nulles ne sont pas affichées.';
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'arp-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'arp-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'arp-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'arp-advanced-list arp-details-inline';
                ...
                [
                    ...
                    box.appendChild(textSpan(item[0], 'arp-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'arp-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('arp-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'arp-hour arp-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'arp-snow-risk arp-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'arp-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'arp-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'arp-stick arp-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'arp-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'arp-details-cell arp-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'arp-details-button';
                ...
                snowAdvanced.className = 'arp-snow-profile arp-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'arp-profile-section';
                    section.appendChild(textSpan(titleText, 'arp-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'arp-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'arp-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'arp-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'arp-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics ARPEGE', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('arp-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • MUCAPE et réflectivité ARPEGE 0,01°.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('arp-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige ARPEGE 0,01°.';
                }
            }
            ...
            if (altitudeLine) {
                altitudeLine.textContent = finite(altitude)
                    ? 'Altitude de ' + cityName + ' : ≈ ' + formatNumber(altitude, 0) + ' m (point de grille ARPEGE)'
                    : 'Altitude de ' + cityName + ' : — (relancez le workflow GitHub ARPEGE v1.0.0)';
                altitudeLine.classList.toggle('arp-altitude-missing', !finite(altitude));
            }
            selectedMapFocus = {
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'arp-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'arp-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'arp-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'arp-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue ARPEGE');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'arp-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'arp-result-name');
        ...
        if (window.ARP_AUTOHEAL && window.ARP_AUTOHEAL.url) {
            fetch(window.ARP_AUTOHEAL.url, { method: 'POST', cache: 'no-store' }).catch(function () {});
        }

        if (!baseUrl) {
            showTableMessage('Adresse des données ARPEGE non configurée.', true);
            return;
        }
        loadIndex()
            .then(function () {
    ...
    whenReady(function () {
        document.querySelectorAll('[data-arp-app]').forEach(initApp);
    });
}());
