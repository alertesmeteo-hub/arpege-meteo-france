#!/usr/bin/env python3
"""Produit des cartes WebP depuis la grille native ARPEGE Europe.
...
class ArpegeMapRenderer:
    """Rend les champs ARPEGE natifs et les frontières cartographiques."""
    ...
    def __init__(
        ...
        *,
        ...
        if not self.pregridded and len(self.latitudes) < 4:
            raise ValueError("Au moins quatre points ARPEGE sont nécessaires")
    ...
    def _interpolate(self, values: np.ndarray) -> np.ndarray:
        ...
        if self.pregridded:
            ...
            raise ValueError(
                "Le champ ARPEGE préinterpolé ne correspond pas à la carte"
            )
        if source.shape != self.latitudes.shape:
            raise ValueError("Le champ ne correspond pas à la grille ARPEGE native")
    ...
    def _image_from_field(self, field: np.ndarray, spec: LayerSpec) -> Image.Image:
        ...
        # Les cartes ARPEGE de référence utilisent des plages colorées nettes,
